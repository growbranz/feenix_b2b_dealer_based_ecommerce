...
const dealerStatusOptions = statusOptions.filter((o) =>
  ["all", "PENDING", "ASSIGNED", "ACCEPTED", "REJECTED", "COMPLETED"].includes(o.value)
)

interface DealerEnquiryListProps {
  initialEnquiries: DealerEnquiryListItem[]
}

export function DealerEnquiryList({ initialEnquiries }: DealerEnquiryListProps) {
  ...
  }, [initialEnquiries, search, statusFilter, priorityFilter])
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Enquiries assigned to you.</p>
        </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        <SearchInput placeholder="Search by product, buyer or business..." value={search} onChange={(e) => setSearch(e.target.value)} />
        <FilterSelect value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} options={dealerStatusOptions} className="md:w-44" />
      ...
      {isPending ? (
        ...
        <EmptyState
          ...
          description="Enquiries buyers make about your products will show up here."
        />
      ) : filtered.length === 0 ? (
        <EmptyState
          icon={Inbox}
              ...
              <tbody>
                {paginated.map((e) => (
                  <tr key={e.id} className="border-b border-slate-100 transition-colors hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700">{e.buyer.name}</td>
